...
import org.jfree.ui.TextAnchor;
    ...
    public class MakeChart {
        ...
        private static XYDataset createDataset(ArrayList<Float> scores) {
            LabeledXYDataset ds = new LabeledXYDataset();

            for (int i = 0; i < scores.size(); i++)
            ds.add(i,  scores.get(i), "x");
         /*   ds.add(12, 68, "A");
            ds.add(13, 65, "B");
            ds.add(14, 67, "C");
            ds.add(15, 69, "D");
        ...
        private static JFreeChart createChart(final XYDataset dataset) {
            NumberAxis domain = new NumberAxis("Iteratii");
            NumberAxis range = new NumberAxis("Scor");
            ...
            JFreeChart chart = new JFreeChart(
                    "Scoruri", JFreeChart.DEFAULT_TITLE_FONT, plot, false);
            return chart;
        }

        void make (ArrayList <Float> scores) {
            JFrame f = new JFrame();
            f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            XYDataset dataset = createDataset(scores);
            JFreeChart chart = createChart(dataset);
            ChartPanel chartPanel = new ChartPanel(chart) {

                @Override
                public Dimension getPreferredSize() {
                    return new Dimension(1320,700);
                }
            };
            f.add(chartPanel);
            f.pack();
